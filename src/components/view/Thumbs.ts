...
export default class ThumbView {
  ...
  constructor(observer: Observer, type: string, orientation: string) {
    ...
    if (this.type === 'range') {
      this.from = new SliderElement('free-slider__thumb--from');
      this.to = new SliderElement('free-slider__thumb--to');
    } else {
      this.single = new SliderElement('free-slider__thumb');
    }

    this.observer.subscribe('clickToTrack', this.changeThumbsOnClick);
    this.observer.subscribe('moveThumbs', this.moveThumbsOnClickToTrack);
  }
